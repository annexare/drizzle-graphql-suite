...
`@drizzle-graphql-suite/schema` includes code generation functions for producing SDL, TypeScript types, and entity definitions from a GraphQL schema.
...
```ts
import { buildSchemaFromDrizzle, generateTypes } from 'drizzle-graphql-suite/schema'
import * as drizzleSchema from './db/schema'
...
```ts
import { buildSchemaFromDrizzle, generateEntityDefs } from 'drizzle-graphql-suite/schema'
import * as drizzleSchema from './db/schema'
