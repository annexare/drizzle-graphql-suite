...
`@drizzle-graphql-suite/client` — Type-safe GraphQL client with entity-based API for drizzle-graphql-suite servers.
...
```ts
import { GraphQLClientError } from 'drizzle-graphql-suite/client'
...
```ts
import { NetworkError } from 'drizzle-graphql-suite/client'
